...
// ============================================================
// DirectoryUploadDownloadService — 目录打包上传 / 下载解压服务
// ============================================================
...
impl DirectoryUploadDownloadService {
    ...
    pub async fn create_snapshot_archive(
        ...
        let checksum = self.upload_dir_as_tar_zst(bucket, &archive_key, src_dir).await?;
    ...
    pub async fn download_and_extract_tar_zst(
        ...
        let manifest = download_manifest(self.object_store.as_ref(), bucket, &manifest_key).await?;
...
mod tests {
    ...
    async fn test_upload_dir_as_tar_zst_success() {
        ...
        let result = service.upload_dir_as_tar_zst(bucket, key, dir.path()).await;
        ...
        // 4. 数据确实写入了内存存储
        let stored = service.object_store.get_object(bucket, key).await.unwrap();
