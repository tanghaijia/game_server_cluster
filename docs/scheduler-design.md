...
**算例**（实例 region=sg、上次运行在 N2、带宽 request 双向 100Mbps；权重取默认；假设 HistoryUtil=0——窗口无采样，history 项每节点贡献 w_history=0.6）：

| 节点 | region | 带宽余量 | locality | history | 利用率 | 健康 | 得分 |
|---|---|---|---|---|---|---|---|
| N1 (sg) | 1 | 0.5 | 0 | 0 | 0.6 | healthy | 1.0 + 0.4 + 0 + 0.6 + 0.28 = **2.28** |
| N2 (us) | 0 | 0.8 | 1 | 0 | 0.2 | healthy | 0 + 0.64 + 0.5 + 0.6 + 0.56 = **2.30** |
| N3 (sg) | 1 | 0.3 | 0 | 0 | 0.1 | degraded | 1.0 + 0.24 + 0 + 0.6 + 0.63 − 2.0 = **0.47** |

结果：选 **N2**——区域不匹配（D3 偏好，不加分但不排除），但带宽余量大 + 数据本地性 + 低负载综合胜出；N3 因 degraded 被惩罚垫底。该例同时演示"区域是偏好不是约束"：区域不匹配的节点依然可能被选中。（与实现 `scheduler_test.go: TestComputeScore_Example62` 对应。）
